...
adversary_PGD_Linf_unseen = attacks.LinfPGDAttack(
    model, loss_fn=nn.CrossEntropyLoss(reduction="sum"), eps=16/255,
    nb_iter=70, eps_iter=2/255, rand_init=True, clip_min=0.0,
    clip_max=1.0, targeted=False)

adversary_deepfool_unseen = DeepfoolLinfAttack(
        model, num_classes=10, nb_iter=50, eps=32/255, clip_min=0.0, clip_max=1.0)
...
if resume:
    checkpoint = torch.load("experiments/CIFAR10/ResNet18/train_MSD_lr_init_0.01/model_AIT_ERM_25.pt")
...
TRAINED_MODEL_PATH = "experiments/CIFAR10/ResNet18/pretrain_MSD/pretrained_MSD_"
if is_REx:
...
#     writer.add_scalar('Validation_accuracy_CW', valid_acc_per_domain[4], epoch+1)
#     writer.add_scalar('Validation_loss_CW', list_losses_epoch_valid[4], epoch+1)
    
#     writer.add_scalar('Validation_accuracy_Deepfool', valid_acc_per_domain[5], epoch+1)
#     writer.add_scalar('Validation_loss_Deepfool', list_losses_epoch_valid[5], epoch+1)
